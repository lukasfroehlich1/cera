...
describe("Matching", () => {
    ...
    describe("#validSingleMatch()", () => {
        it("should produce 1 result. simple case", (done) => {
            ...
            let driver1 = new Driver(1, 11, new Date("2011-12-01 00:00:00"), new Time(15, 0), new Time(17, 0), [], sf, cp, 1600, 20, 3);
        ...
        it("should produce 1 result. Date has leftover time", (done) => {
            ...
            let driver1 = new Driver(1, 11, new Date("2011-12-01 00:23:00"), new Time(13, 21), new Time(17, 0), [], sf, cp, 1200, 20, 3);
             map_riders_to_drivers([rider1], [driver1], (err: any, res: Array<Match>) => {
                 if (err) { done(err); }
                 expect(res).to.have.length(1);
                 done();
    ...
    describe("#timeChecks()", () => {
        it("should produce no results. times dont overlap. using minute form", (done) => {
            ...
            let driver1 = new Driver(1, 11, new Date("2011-12-01 00:00:00"), new Time(0), new Time(758), [], sf, cp, 1200, 20, 3);
             map_riders_to_drivers([rider1], [driver1], (err: any, res: Array<Match>) => {
                 if (err) { done(err); }
        ...
        it("should produce no results. times dont overlap", (done) => {
            let rider1 = new Rider(1,
                                   ...
                                   [sj],
                                   cp);
            let driver1 = new Driver(1, 11, new Date("2011-12-01 00:00:00"), new Time(15, 21), new Time(17, 0), [], sf, cp, 1200, 20, 3);
             map_riders_to_drivers([rider1], [driver1], (err: any, res: Array<Match>) => {
                 if (err) { done(err); }
                 expect(res).to.have.length(0);
                 done();
        ...
        it("should produce no results. months dont overlap", (done) => {
            ...
            let driver1 = new Driver(1, 11, new Date("2011-12-01 00:00:00"), new Time(13, 21), new Time(17, 0), [], sf, cp, 1200, 20, 3);
             map_riders_to_drivers([rider1], [driver1], (err: any, res: Array<Match>) => {
                 if (err) { done(err); }
                 expect(res).to.have.length(0);
                 done();
        ...
        it("should produce no results. dates dont overlap extra time", (done) => {
            let rider1 = new Rider(1, 10, new Date("2011-12-02 00:00:00"), new Time(14, 34), new Time(15, 21), [sj], cp);
            let driver1 = new Driver(1, 11, new Date("2011-12-01 20:00:00"), new Time(13, 21), new Time(17, 0), [], sf, cp, 1200, 20, 3);
             map_riders_to_drivers([rider1], [driver1], (err: any, res: Array<Match>) => {
                 if (err) { done(err); }
                 expect(res).to.have.length(0);
                 done();
